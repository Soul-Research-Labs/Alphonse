/**
 * Vault flow — Confirm Shield: review & execute shield (Public → Vault).
 *
 * This is a signing approval screen — never auto-signs.
 */
...
export default function ConfirmShieldScreen() {
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      ...
      const result = await privacyAdapter.shield({
        from: state.address,
      ...
      // Refresh both public and vault balances
      ...
      setError('Shield failed. Check network and try again.');
    } finally {
      setProcessing(false);
    }
  }, [
  ...
  return (
    <SafeAreaView edges={['top', 'bottom']} style={{ flex: 1, backgroundColor: '#f0f4ff' }}>
      <ScrollView className="flex-1 px-6 pt-8">
        <Text className="text-2xl font-bold text-brand-900">Confirm Shield</Text>
        <Text className="mt-2 text-sm text-brand-700">Public → Vault (Private)</Text>
        ...
        <SectionCard title="Details" subtitle="Review before confirming">
          <View className="mt-2 gap-2">
            ...
            <View className="flex-row justify-between">
              ...
              <Text className="text-sm text-brand-900">Public (Checking)</Text>
            </View>
            <View className="flex-row justify-between">
        ...
        {!privacyAdapter && (
          <Warning message="Privacy adapter is not available. Shield is disabled." />
        )}
