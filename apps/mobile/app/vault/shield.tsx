/**
 * Vault flow — Shield: enter amount to move from Public → Vault.
 */
...
export default function ShieldScreen() {
  ...
  const assets = useMemo<AssetOption[]>(() => {
    const list: AssetOption[] = [
      { symbol: 'ETH', decimals: 18, balance: state.balanceFormatted ?? '0', tokenAddress: null },
    ];
  ...
  return (
    <SafeAreaView edges={['top', 'bottom']} style={{ flex: 1, backgroundColor: '#f0f4ff' }}>
      <ScrollView className="flex-1 px-6 pt-8" keyboardShouldPersistTaps="handled">
        ...
        <Text className="mt-4 text-2xl font-bold text-brand-900">Shield</Text>
        <Text className="mt-2 text-sm text-brand-700">
          Move funds from Public to Vault (private)
        </Text>
        ...
        <View className="mt-4 flex-row items-center justify-between">
          <Text className="text-sm text-brand-700">
            Public balance: {selected.balance} {selected.symbol}
          </Text>
